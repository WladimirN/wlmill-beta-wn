...
private:
	...
	enum FlagDrive
               ...
               fl_resetastop=      1<<2 , //сброс после остановки
               //fl_enable=     1<<3 ,
               fl_lastrot=    1<<4 ,
  ...
  QList <double> m_orgPosCountList;

  double m_Vmov=0;  //заданная скорость
  double m_Vnow=0;  //текущая скорость
...
inline double getLatch2Pos(quint8 i=0) {return (double)getLatch2PosL(i)*dimension();}
inline double getLatch3Pos(quint8 i=0) {return (double)getLatch3PosL(i)*dimension();}
 ...
 double getHomePosition()         {return m_homePosition;}
...
virtual	void resetAuto();
        ...
        void setResetAfterStop(bool set=true) {qDebug()<<"setResetAfterStop"<<set; Flag.set(fl_resetastop,set);}
private :
...
public:
    bool getInput(typeInputAxis type,bool all=false);
...
public:
   ...
   void startMovVel(bool dir,float V)   {if(setVel(dir))  startMotion(V);}
...
virtual	void startManualInc(double k) {startMovManual( qAbs(k));}
virtual	void startManualDec(double k) {startMovManual(-qAbs(k));}
...
public:
   void startMovManual(double rot,double dist=0);
