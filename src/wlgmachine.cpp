...
void WLGMachine::stopMov() //полна€ остановка
{
...
/*
  if(m_motDevice->getModulePWM())
     {   
     setEnablePWMS(isUsePWMS());//устанавливаем св€зь
...
case probeXY:
              switch(iOperation)
              {
               case 0: if(getInProbe())
                        ...
                        Planner->clear(); //едем к точке и опускаемс€
                        Planner->resetProbe();
              ...
              case 5: if(Planner->isProbe2()||getInProbe()) {
                        setMessage(tr("error sensor signal"),"inProbe",-5);//если наткнулись на прип€тствие при подходе то ошибка
                        }else{
              ...
              case 10: if(Planner->isProbe2()){  //если было касание
                         ...
                         if(SGProbe::backDist<0){
                            ...
                            Planner->resetProbe();
                            Planner->setActInProbe(SGProbe::typeStop); //остановимс€ при отскоке
...
&&!isRunMScript())
 ...
 while(MillTraj.size()<100
   ...
   if(!baseTraj.isEmpty()
     &&baseTraj.first().isScript()) //если первый элемент Script то отправл€ем его на исполнение
     {
   ...
   if((isimpli+1)<baseTraj.size()) //если сгладили и дошли до точки, но не до конца
    ...
    baseTraj=baseTraj.mid(isimpli+1); //оставл€ем один элемент на будущее, может и Script
    }   
    else if(m_iProgram==(m_Program->getElementCount())      //до конца
          ||WLElementTraj::detectScript(baseTraj)) //до конца
          {
          while(!baseTraj.isEmpty())  //перемещаем всЄ + один Script
           {
...
case WLElementTraj::script: qDebug()<<"Script"<<isActiv()<<isRunMScript()<<(ME.index==ModulePlanner->getCurIdElement())<<ME.escript.singleRun;
                            ...
                            if(!isActiv()  //стоит
                           ||(!isRunMScript()
                             &&ME.index==ModulePlanner->getCurIdElement())  //или порождЄн текущим элементом
...
case WLElementTraj::line:  {
                           ...
                           //(m_mainDim*60.0/ME.speedF) обратный коэф дл€ будущей подстановки
