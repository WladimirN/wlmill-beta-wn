...
const float hypB=hypotf(M(0,0),M(1,0));

b=atan2(-M(2,0),hypB)*180.0/M_PI;

if(hypB==0.0f) // иначе не ловит 90 градусов
    {
    a=0;
    c=b/qAbs(b)*atan2(M(0,1),M(1,1))*180.0/M_PI;
    }
else
    {
    a=atan2(M(1,0),M(0,0))*180.0f/M_PI;
    c=atan2(M(2,1),M(2,2))*180.0f/M_PI;
    }
...
QString toString(int xyz=3,int an=10,int en=10,bool all=false)
{
QString ret;

ret=
     "X "+QString::number(fr.x,'f',xyz)
   +",Y "+QString::number(fr.y,'f',xyz)
   +",Z "+QString::number(fr.z,'f',xyz)
...
WL6APos   eAPoint;//конечная точка
