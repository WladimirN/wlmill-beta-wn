...
void WLIOWidget::setDCTableEncoder(QModelIndex mindex)
{
int row=mindex.row();
...
case 0:
       if(QMessageBox::question(this,tr("Question")
                                ,(m_encoderViewModel->moduleEncoder()->getEncoder(row)->isEnable()
                                ? tr("set disable encoder")
                                : tr("set enable encoder"))
                                +" WLMotion "+QString::number(row)+" ?",QMessageBox::Yes|QMessageBox::No)==QMessageBox::Yes)
         {
         m_encoderViewModel->moduleEncoder()->getEncoder(row)->setEnable(true);
         }
        break;
}
}
