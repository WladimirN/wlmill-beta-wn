...
void WLPamListWidget::setFminutes(bool Fminutes)
{
...
QString uV=m_unit+"/"+(m_Fminutes? tr("m"):tr("s"));
...
for(int i=0;i<ui->tableWidget->rowCount();i++)
 ...
 item = new  QTableWidgetItem;
 item->setFlags(Qt::ItemIsEnabled);
 item->setText(QString::number(m_Fminutes ? padList[i].Vst*60.0:padList[i].Vst));
 ...
 item = new  QTableWidgetItem;
 item->setFlags(Qt::ItemIsEnabled);
 item->setText(QString::number(m_Fminutes ? padList[i].Vma*60.0 : padList[i].Vma));
