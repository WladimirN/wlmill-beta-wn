...
Item {
    ...
    var file=MSCRIPT.getValueStr("WLProbeFileINI")

    sbDiam.value =2*FILE.loadValue(file,"Circ4XYDialog/R",50);
    sbA1.value   =FILE.loadValue(file,"Circ4XYDialog/A",0);
    ...
    var Depth=MSCRIPT.getValueNum("WLProbeXYbaseZ")-sbDepth.value

    var file=MSCRIPT.getValueStr("WLProbeFileINI")

    FILE.saveValue(file,"Circ4XYDialog/R",R);
    FILE.saveValue(file,"Circ4XYDialog/A",A);
    FILE.saveValue(file,"lastDialog/Depth",Depth);

   // MSCRIPT.runScript("WLProbeCirc4XY("+X+","+Y+","+A+","+R+","+Depth+")");
    }
    }
    ...
    Connections {
    target: loader_probePoint4.item
    onSignalChangedAngle: root.a1=angle
    }
    ...
    loader_probePoint3.item.angle=root.a1
    loader_probePoint4.item.angle=root.a1
    sbA1.value=root.a1
    }
