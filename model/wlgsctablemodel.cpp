...
if (role == Qt::DisplayRole || role == Qt::EditRole) {
     if(headers[index.column()]=="index")
        return mGCode->getSCGStr(mGCode->getDataSC()->getValueAt(index.row(),headers[index.column()],"").toInt());
     }

if (role == Qt::ToolTipRole) {
     if(headers[index.column()]=="index")
        return mGCode->getDataSC()->getValueAt(index.row(),headers[index.column()],"").toString();
     }
